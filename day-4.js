const solvePuzzle = (input) => {
  const transformedInput = tranformInput(input);
  
  const sleepMap = {};

  let activeElf;
  let fellAsleep;
  for (let { date, action } of transformedInput) {
    if (action.indexOf('begins shift') > -1) {
      activeElf = action.replace('Guard #', '').replace(' begins shift', '');
    
    } else if (action.indexOf('falls asleep') > -1) {
      fellAsleep = date;
    
    } else if (action.indexOf('wakes up') > -1) {
      if (!sleepMap[activeElf]) {
        sleepMap[activeElf] = {
          total: 0,
          minutes: {}
        };
      ...
      for (let d = new Date(fellAsleep); d < date; d.setMinutes(d.getMinutes() + 1)) {
        sleepMap[activeElf].total++;
        
        const minute = d.getMinutes();
        sleepMap[activeElf].minutes[minute]
          = sleepMap[activeElf].minutes[minute]
            ? sleepMap[activeElf].minutes[minute] + 1 : 1
      }
    }
  }

  const elfIdWithHighestTotal = Object.keys(sleepMap).sort((a, b) => {
    return sleepMap[b].total - sleepMap[a].total;
  })[0];

  const elf = sleepMap[elfIdWithHighestTotal];
  const minuteWithMostSleep = Object.keys(elf.minutes).sort((a, b) => {
    return elf.minutes[b] - elf.minutes[a];
  })[0];

  return minuteWithMostSleep * elfIdWithHighestTotal;
}

const tranformInput = (input) => {
  return input
    .split('\n')
    .map((row) => {

    const parts = row.split(/\[|\]/);
    const date = new Date(parts[1].replace('1518', '2018'));
...
const CORRECT_ANSWER = 30630;
const INPUT = `[1518-08-08 00:45] falls asleep
