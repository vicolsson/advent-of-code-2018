const solvePuzzle = (input) => {
  ...
  while (allSteps.length > 0) {
    ...
    const valid = allSteps.filter(s => !rows.some(r => r.after === s))[0];

    result += valid;

    allSteps.splice(allSteps.indexOf(valid), 1);
    rows = removeDependenciesForStep(rows, valid);
  }
...
const CORRECT_ANSWER = 'BFLNGIRUSJXEHKQPVTYOCZDWMA';
