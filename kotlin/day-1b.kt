fun solvePuzzle(): String {
  ...
  var i = 0;
  var sum = 0;
  val previousSums: MutableList<String> = ArrayList()
  while (true) {
    val valToAdd = inputRows[i % inputRows.size]
    sum += valToAdd
    val stringSum = sum.toString()

    i++;

    if (previousSums.contains(stringSum)) {
      return stringSum
    }
    previousSums.add(stringSum)
  }
}
