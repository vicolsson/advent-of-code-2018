const solvePuzzle = (input) => {
  ...
  let maximum = { asleep: -1 };
  for (let elfId of Object.keys(sleepMap)) {
    for (let minute of Object.keys(sleepMap[elfId].minutes)) {
      const asleep = sleepMap[elfId].minutes[minute];
      if (asleep > maximum.asleep) {
        maximum = {
          elfId,
          minute,
          asleep
  ...
  return maximum.elfId * maximum.minute;
}
